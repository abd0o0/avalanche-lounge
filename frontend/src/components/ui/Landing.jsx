  ...
  return (
    <div className="bg-white dark:bg-gray-950 dark:text-white overflow-hidden">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, x: 80, rotate: 6 }} animate={{ opacity: 1, x: 0, rotate: 6 }} transition={{ delay: 1, duration: 0.8 }}
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          ...
          <motion.p variants={fadeUp} initial="hidden" animate="visible" custom={2}
            className="text-3xl md:text-5xl text-cyan-200/80 mb-6" style={{ fontFamily: '"Noto Naskh Arabic", serif' }}>
            {t('brand.slogan')}
          </motion.p>
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={4}
            ...
            <Link to="/reservation" className="px-8 py-3.5 bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-semibold rounded-xl hover:scale-105 transition-transform shadow-lg shadow-cyan-500/20">
              {t('home.reserve')}
            </Link>
            <Link to="/menu" className="px-8 py-3.5 bg-white/10 backdrop-blur-sm text-white font-semibold rounded-xl border border-white/20 hover:bg-white/20 transition-all">
              {t('home.viewMenu')}
            </Link>
          </motion.div>
        </div>
      ...
      <section className="py-24 px-4 bg-white dark:bg-gray-950">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="relative h-96 lg:h-[500px]">
            <motion.img initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}
              viewport={{ once: true }}
              src={lounge} alt="Lounge"
              className="absolute inset-0 w-4/5 h-full object-cover rounded-2xl shadow-2xl"
            />
            <motion.img initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.2 }}
              viewport={{ once: true }}
              src={food} alt="Food"
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}
            viewport={{ once: true }}>
            <p className="text-cyan-500 text-xs tracking-[0.3em] uppercase font-semibold mb-3">{t('home.aboutTitle')}</p>
            <h2 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-6 leading-tight">
              {t('home.aboutTitleText')}
            </h2>
      ...
      <section className="py-20 px-4 bg-gray-50 dark:bg-gray-900">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-cyan-500 text-xs tracking-[0.3em] uppercase font-semibold mb-2">{t('home.offersTitle') || 'What We Offer'}</p>
            <h2 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white">
              {t('home.experienceTitle') || 'Experience Avalanche'}
            </h2>
          </div>
      ...
      <section className="relative py-28 px-4 overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          <h2 className="text-4xl md:text-6xl font-bold text-white mb-4">{t('home.ctaBannerTitle')}</h2>
          <p className="text-gray-300 text-lg mb-8">{t('home.ctaBannerText')}</p>
          <Link to="/reservation" className="inline-block px-10 py-4 bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-bold rounded-xl text-lg hover:scale-105 transition-transform shadow-2xl shadow-cyan-500/30">
            {t('home.reserve')} →
          </Link>
        </div>
      </section>
